...
export class DeduplicateDriverProfile1778774590456 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP INDEX IF EXISTS "public"."idx_vehicles_device_id"`);
    await queryRunner.query(`ALTER TABLE "drivers" DROP COLUMN IF EXISTS "full_name"`);
    await queryRunner.query(`ALTER TABLE "drivers" DROP COLUMN IF EXISTS "phone"`);
    await queryRunner.query(`ALTER TABLE "drivers" DROP COLUMN IF EXISTS "avatar_url"`);
    ...
    await queryRunner.query(`ALTER TABLE "vehicles" ADD COLUMN IF NOT EXISTS "device_id" character varying UNIQUE`);
    await queryRunner.query(`ALTER TABLE "vehicles" ADD COLUMN IF NOT EXISTS "model" character varying`);
    await queryRunner.query(`ALTER TABLE "vehicles" ADD COLUMN IF NOT EXISTS "year" integer`);

    // Add missing fingerprint_id column to drivers
    await queryRunner.query(`ALTER TABLE "drivers" ADD COLUMN IF NOT EXISTS "fingerprint_id" character varying UNIQUE`);

    // Add missing pickup_actual_location and delivery_actual_location columns to orders
    await queryRunner.query(`ALTER TABLE "orders" ADD COLUMN IF NOT EXISTS "pickup_actual_location" geography(Point, 4326)`);
    await queryRunner.query(`ALTER TABLE "orders" ADD COLUMN IF NOT EXISTS "delivery_actual_location" geography(Point, 4326)`);

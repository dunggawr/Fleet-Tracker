...
export default function LiveTrackingPage() {
  ...
  const initSocket = useCallback(async () => {
    ...
    socket.on(SOCKET_EVENTS.GPS_UPDATE, (data: any) => {
      setVehicles(prev => {
        ...
        const update: VehiclePosition = {
          ...
          licensePlate: data.licensePlate || data.license_plate || data.plateNumber || existing?.licensePlate || `VH-${data.vehicleId.slice(0, 6)}`,
          driverName: data.driverName || existing?.driverName || 'Unknown Driver',
          ...
          ordersCount: data.ordersCount !== undefined ? data.ordersCount : existing?.ordersCount,
        };

        if (existing) {
          return prev.map(v => v.vehicleId === update.vehicleId ? update : v);
        }
        return [...prev, update];
      });
    });
    ...
    socket.on(SOCKET_EVENTS.DRIVER_STATUS_CHANGED, (data: any) => {
      setVehicles(prev =>
        prev.map(v =>
          v.driverId === data.driverId ? { ...v, status: statusMap[data.status] || v.status } : v
        )
      );
    });
      ...
      try {
        ...
        const initial: VehiclePosition[] = vehiclesData.map((v: any) => {
          ...
          return {
            ...
            latitude: v.lastKnownLocation?.coordinates?.[1] || 10.762622 + (Math.random() - 0.5) * 0.05,
            longitude: v.lastKnownLocation?.coordinates?.[0] || 106.660172 + (Math.random() - 0.5) * 0.05,
            speed: 0,
            heading: 0,
    ...
    <div className="h-full flex flex-col gap-4 p-4 lg:p-5 bg-background min-h-0 overflow-y-auto lg:overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0">
        <div className="flex items-center gap-3 text-text">
          ...
          <div>
            <h1 className="text-2xl font-bold m-0 tracking-tight">Giám sát hành trình</h1>
            <div className="flex items-center gap-2 mt-1">
              <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${isConnected ? 'bg-success/15 text-success' : 'bg-danger/15 text-danger'}`}>
                <div className={`w-1.5 h-1.5 rounded-full ${isConnected ? 'bg-success animate-pulse' : 'bg-danger'}`} />
                {isConnected ? 'Live' : 'Offline'}
              </div>
              <span className="text-text-dim text-xs">•</span>
              <span className="text-text-dim text-xs">{stats.total} phương tiện</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 bg-surface-low p-1 rounded-xl border border-border">
          {(['all', 'active', 'idle', 'offline'] as const).map(f => (
            ...
            >
              ...
              <span className={`px-1.5 py-0.5 rounded-md text-[10px] font-bold ${filter === f ? 'bg-white/20' : 'bg-surface-highest'}`}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 shrink-0">
        <div className="flex flex-col gap-1 p-4 bg-surface rounded-2xl border border-border relative overflow-hidden group">
          ...
          <span className="text-text-dim text-xs font-medium uppercase tracking-wider">Tổng đội xe</span>
          <div className="text-3xl font-bold text-text tabular-nums">{stats.total}</div>
        ...
        <div className="flex flex-col gap-1 p-4 bg-surface rounded-2xl border border-border border-l-4 border-l-success relative overflow-hidden group">
          ...
          <span className="text-text-dim text-xs font-medium uppercase tracking-wider">Đang vận hành</span>
          <div className="text-3xl font-bold text-text tabular-nums text-success">{stats.active}</div>
          <div className="flex items-center gap-1 text-[10px] text-success mt-1">
            <div className="w-1.5 h-1.5 rounded-full bg-success animate-ping" /> Real-time active
          </div>
        </div>
        <div className="flex flex-col gap-1 p-4 bg-surface rounded-2xl border border-border border-l-4 border-l-warning relative overflow-hidden group">
          ...
          <span className="text-text-dim text-xs font-medium uppercase tracking-wider">Đang chờ việc</span>
          <div className="text-3xl font-bold text-text tabular-nums text-warning">{stats.idle}</div>
          <div className="text-[10px] text-text-dim mt-1">Sẵn sàng điều động</div>
        </div>
        <div className="flex flex-col gap-1 p-4 bg-surface rounded-2xl border border-border border-l-4 border-l-danger relative overflow-hidden group">
          ...
          <span className="text-text-dim text-xs font-medium uppercase tracking-wider">Cảnh báo hệ thống</span>
          <div className="text-3xl font-bold text-text tabular-nums text-danger">{stats.alerts}</div>
          <div className="text-[10px] text-danger/80 mt-1">Yêu cầu xử lý ngay</div>
        </div>
      </div>

      <div className="flex flex-col lg:grid lg:grid-cols-[1fr_360px] gap-4 flex-1 min-h-0 lg:max-h-[75vh]">
        {/* Map Container */}
          ...
          {selectedVehicle && (
            <div className="absolute bottom-4 left-4 right-4 sm:right-auto sm:w-[320px] bg-surface/90 backdrop-blur-xl rounded-2xl border border-white/10 p-5 border-t-4 border-t-primary shadow-[0_20px_50px_rgba(0,0,0,0.3)] animate-in slide-in-from-bottom-5 duration-300 z-20">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[11px] font-bold text-text-dim m-0 uppercase tracking-[0.2em]">Chi tiết phương tiện</h3>
                <button 
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-inner ${
                    ...
                    selectedVehicle.status === 'idle' ? 'bg-warning/20 text-warning' :
                    'bg-text-dim/20 text-text-dim'
                  }`}>
                    <Truck size={24} />
                  </div>
                  <div className="min-w-0">
                    <div className="text-lg font-black text-text leading-tight tracking-tight truncate">{selectedVehicle.licensePlate}</div>
                    <div className="flex items-center gap-2 mt-1">
                      <div className={`w-1.5 h-1.5 rounded-full ${
                        selectedVehicle.status === 'active' ? 'bg-success animate-pulse' :
                        selectedVehicle.status === 'idle' ? 'bg-warning' :
                        'bg-text-dim'
                      }`} />
                      <span className="text-[10px] font-bold text-text-dim uppercase">{selectedVehicle.status}</span>
                    </div>
                  </div>
                </div>

                <div className="grid grid-cols-1 gap-2.5">
                  <DetailItem label="Tài xế" value={selectedVehicle.driverName} icon={<Activity size={14} />} />
                  <div className="grid grid-cols-2 gap-2.5">
                    <DetailItem label="Tốc độ" value={`${selectedVehicle.speed.toFixed(0)} km/h`} icon={<Navigation size={14} />} />
                    <DetailItem label="Cập nhật" value={formatDistanceToNow(new Date(selectedVehicle.lastUpdate), { addSuffix: false })} icon={<Clock size={14} />} />
                  </div>
                </div>
          ...
          {alerts.filter(a => !a.resolved).length > 0 && (
            <div className="glass rounded-2xl p-4 border-l-4 border-l-danger shadow-xl animate-in slide-in-from-right duration-300">
              <h3 className="flex items-center justify-between text-sm font-bold text-text mb-4 mt-0">
                ...
                <span className="bg-danger text-white text-[10px] px-2 py-0.5 rounded-full">NEW</span>
              </h3>
              <div className="flex flex-col gap-2 max-h-[160px] overflow-y-auto pr-1">
                {alerts.filter(a => !a.resolved).map(alert => (
                  <div key={alert.id} className="flex items-start gap-3 p-3 bg-surface-highest/40 backdrop-blur-md rounded-xl border border-white/5 group">
                    <div className="flex-1 flex flex-col gap-1">
                      <span className="text-text text-xs leading-relaxed font-medium">{alert.message}</span>
            ...
            <div className="flex-1 overflow-y-auto p-2 flex flex-col gap-1.5 custom-scrollbar">
              {filteredVehicles.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 px-5 text-center">
                  ...
                  <div className="text-sm font-medium text-text">Không tìm thấy xe</div>
                  <div className="text-xs text-text-dim mt-1">Thay đổi bộ lọc để xem thêm</div>
                </div>
              ) : (
                filteredVehicles.map(vehicle => (
                    <div
                      key={vehicle.vehicleId}
                      className={`group flex items-center gap-4 p-4 rounded-2xl border transition-all duration-500 cursor-pointer relative overflow-hidden ${
                        selectedVehicle?.vehicleId === vehicle.vehicleId
                          ? 'bg-primary/15 border-primary/40 shadow-[0_10px_30px_rgba(59,130,246,0.15)] translate-x-1 ring-1 ring-primary/20'
                          : 'bg-surface-low/40 border-white/5 hover:bg-surface-high/60 hover:border-border/50 hover:-translate-y-0.5 shadow-sm'
                      }`}
                      onClick={() => setSelectedVehicle(
                        selectedVehicle?.vehicleId === vehicle.vehicleId ? null : vehicle
                      )}
                    >
                      {/* Subtle Background Glow for Active Status */}
                      ...
                      <div className="relative">
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center relative z-10 transition-all duration-500 group-hover:scale-110 shadow-lg ${
                          vehicle.status === 'active' ? 'bg-success text-white' :
                          vehicle.status === 'idle' ? 'bg-warning/20 text-warning' :
                          'bg-text-dim/20 text-text-dim'
                        }`}>
                          <Truck size={24} />
                        </div>
                        <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-surface z-20 ${
                          vehicle.status === 'active' ? 'bg-success' :
                          vehicle.status === 'idle' ? 'bg-warning' :
                          'bg-text-dim'
                        }`} />
                        {vehicle.status === 'active' && (
                          <div className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-success animate-ping opacity-75" />
                        )}
                      </div>

                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <div className="text-[15px] font-black text-text truncate tracking-tight group-hover:text-primary-light transition-colors">
                            {vehicle.licensePlate}
                          </div>
                          {vehicle.speed > 0 ? (
                            <div className="flex items-center gap-1 text-[10px] font-bold text-success bg-success/10 px-2 py-0.5 rounded-lg border border-success/20 whitespace-nowrap">
                              <Activity size={10} className="animate-pulse" />
                              {vehicle.speed.toFixed(0)} km/h
                            </div>
                          ) : (
                            <div className="text-[10px] font-bold text-text-dim bg-surface-highest/50 px-2 py-0.5 rounded-lg border border-white/5">
                              Đang dừng
                            </div>
                          )}
                        </div>
                        <div className="text-[11px] text-text-dim truncate mt-0.5 font-semibold uppercase tracking-wide opacity-80">{vehicle.driverName}</div>
                        
                        <div className="flex items-center gap-3 mt-2">
                          <div className="flex items-center gap-1 text-[10px] text-text-dim/70">
                            <MapPin size={11} className="text-primary/60" />
                            <span className="truncate">TP. Hồ Chí Minh</span>
                          </div>
                          {vehicle.ordersCount !== undefined && (
                            <div className="flex items-center gap-1 text-[10px] text-primary-light font-black bg-primary/10 px-2 py-0.5 rounded-md border border-primary/10">
                              {vehicle.ordersCount} đơn
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className={`transition-all duration-300 ${
                      selectedVehicle?.vehicleId === vehicle.vehicleId ? 'opacity-100 scale-100' : 'opacity-0 scale-90 group-hover:opacity-100 group-hover:scale-100'
                    }`}>
...
function DetailItem({ label, value, icon, highlight }: { label: string, value: string, icon: React.ReactNode, highlight?: boolean }) {
  return (
    <div className="flex items-center justify-between p-3 bg-surface-low rounded-xl border border-white/5 transition-colors hover:bg-surface-high">
      ...
      <span className={`text-xs font-bold tabular-nums ${highlight ? 'text-primary' : 'text-text'}`}>{value}</span>
    </div>
  );
}
